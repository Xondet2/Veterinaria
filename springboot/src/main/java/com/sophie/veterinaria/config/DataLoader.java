  ...
  CommandLineRunner seed(UsuarioRepository usuarios, PasswordEncoder encoder) {
    return args -> {
      if (usuarios.findByEmailIgnoreCase("admin@clinic.com").isEmpty()) {
        ...
        admin.setContraseñaHash(encoder.encode("demo123"));
        ...
        vet.setNombre("Ana");
        vet.setApellido("López");
        vet.setContraseñaHash(encoder.encode("demo123"));
        vet.setRol(Usuario.Rol.veterinario);
        vet.setEspecialidad("Pequeños animales");
        vet.setEstado(Usuario.Estado.activo);
        usuarios.save(vet);

        var owner = new Usuario();
        owner.setEmail("owner@example.com");
        owner.setNombre("Juan");
        owner.setApellido("Pérez");
        owner.setContraseñaHash(encoder.encode("demo123"));
        owner.setRol(Usuario.Rol.dueño);
        owner.setEstado(Usuario.Estado.activo);
        usuarios.save(owner);
      }
    };
  }
