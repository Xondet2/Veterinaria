...
@RestController @RequestMapping("/api/vacunas") public class VacunaController {
  ...
  @GetMapping public ResponseEntity<?> list(@RequestParam(required=false) String pet, Authentication auth){
    ...
    if (pet!=null){ var m=mascotas.findById(UUID.fromString(pet)).orElseThrow(); res=vacunas.findByPetOrderByAppliedDateDesc(m);
    } else { res=vacunas.findAll(); }
    if (user.getRole()== Usuario.Rol.dueño) res=res.stream().filter(v->v.getPet().getOwner().getId().equals(user.getId())).toList();
  ...
  @PostMapping public ResponseEntity<?> create(@Validated @RequestBody CreateVacuna req, Authentication auth){
    ...
    var v=new Vacuna(); v.setPet(m); v.setVeterinarian(vet); v.setName(req.getName()); v.setAppliedDate(LocalDate.parse(req.getAppliedDate())); v.setDose(req.getDose()); v.setLot(req.getLot()); v.setNextDate(req.getNextDate()!=null? LocalDate.parse(req.getNextDate()): null);
    var saved=vacunas.save(v); return ResponseEntity.status(201).body(Map.of("success",true,"data",saved));
  }
  @Data static class CreateVacuna { @NotNull private String petId; @NotBlank private String name; @NotBlank private String appliedDate; private String dose; private String lot; private String nextDate; }
}